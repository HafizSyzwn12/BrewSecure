...
use Illuminate\Support\Facades\Cache; // for storing OTP temporarily
...
class UsersController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        try {
            ...
            Cache::put('2fa_' . $user->id, $otp, now()->addMinutes(5));

            // ✅ Send OTP via email
            Mail::to($user->email)->send(new Send2FACode($otp));
    ...
    public function verify2FA(Request $request): JsonResponse
    {
        ...
        \Log::info('Verifying 2FA for user ID: ' . $request->userId);
        \Log::info('Provided OTP: ' . $request->otp);

        $cachedOtp = Cache::get('2fa_' . $request->userId);

        if (!$cachedOtp || $cachedOtp != $request->otp) {
            return response()->json(['error' => 'Invalid or expired verification code'], 401);
        }

        $user = Users::with('role:id,name')->findOrFail($request->userId);
        ...
        Cache::forget('2fa_' . $request->userId);
